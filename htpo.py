...
async def check_otp_code_availability(context: CallbackContext, chat_id: int, api_key: str, id: str, cancel_flag: asyncio.Event) -> None:
    ...
    while not cancel_flag.is_set():
        ...
        print("RESPONDERS:", response)
        print("RESPONDERS:", response.text)
    
        if response.status_code == 200 and "STATUS_OK" in response.text:
            print("i go send message")
            ...
            # api_buttons = [[InlineKeyboardButton("\U0001F4F2 Get 123", callback_data='123'),
            #      InlineKeyboardButton("\U0001F4B0 Generate 456", callback_data='456')],
            #     ]
...
async def service_callback(update: Update, context: CallbackContext) -> None:   
    ...
    api_key = context.user_data.get('api_key')
    print(query.data)
    response_name = requests.get(f'https://smstore.su/stubs/handler_api.php?api_key={api_key}&action=getServices')
    services_name = response_name.json()
    service_id_to_name = {key: value for key, value in services_name.items()}
    ...
    if responded == "NO_BALANCE":
        ...
        # context.user_data['message_id']= update.message.id
        ...
        sent_message = await context.bot.send_message(chat_id=query.message.chat_id, text=f"{message}\n\n{service_name}\nNumber: \U0001F4F1 {access_number}\nID:{main_id}", reply_markup=reply_markup)
        
        cancel_flag = asyncio.Event()
        context.user_data['cancel_flag'] = cancel_flag
        print('first cancel flag:', cancel_flag)
        check_otp_task = asyncio.ensure_future(check_otp_code_availability(context, chat_id, api_key, main_id, cancel_flag))
...
async def cancel_activation(update: Update, context: CallbackContext):
    ...
    query = update.callback_query
    api_key = context.user_data.get('api_key')
    main_id = context.user_data.get('id')
    print('Na main ID:',main_id )
    ...
    print("na d query data:", query.data)
    ...
    if query.data == "otp_code":
        ...
        response = requests.get(f"https://smstore.su/stubs/handler_api.php?api_key={api_key}&action=setStatus&id={main_id}&status=8")
        ...
        response = requests.get(f"https://smstore.su/stubs/handler_api.php?api_key={api_key}&action=setStatus&id={main_id}&status=3")
        await context.bot.send_message(chat_id=query.message.chat_id, text="Sending SMS", reply_markup=reply_markup)
        cancel_flag = asyncio.Event()
        context.user_data['cancel_flag'] = cancel_flag
        print(cancel_flag)
            
        await asyncio.create_task(check_otp_code_availability(context, query.message.chat_id, api_key, main_id, cancel_flag))
    return STATE_CONFIRMATION

    # else:    
    #     response = requests.get(f"https://smstore.su/stubs/handler_api.php?api_key={api_key}&action=setStatus&id={main_id}&status={query.data}")
...
app = ApplicationBuilder().token("5912561023:AAH9OaIMM-bXBEoNlAip0-3ZV28Sc5qMevA").read_timeout(30).write_timeout(30).build()
