...
def save_user_id(chat_id, user_id):
    ...
    try:
        cursor = conn.cursor()
        query = "INSERT INTO users (chat_id, user_id) VALUES (%s, %s) ON CONFLICT (chat_id) DO UPDATE SET user_id = EXCLUDED.user_id"
...
def get_user_id(chat_id):
    ...
    try:
        cursor = conn.cursor()
        select_query = f"SELECT user_id FROM users WHERE chat_id = '{chat_id}'"
        cursor.execute(select_query)
...
def save_user_state(chat_id, user_state):
    conn = psycopg2.connect(
        host="dpg-ci1covndvk4kgoo1fcj0-a.oregon-postgres.render.com",
        port="5432",
        database="telegrambot_dy64",
    ...
    try:
        cursor = conn.cursor()
        query = "INSERT INTO users (chat_id, user_state) VALUES (%s, %s) ON CONFLICT (chat_id) DO UPDATE SET user_state = EXCLUDED.user_state"
        cursor.execute(query, (chat_id, user_state))
        conn.commit()

    except (Exception, Error) as error:
        print("Error while saving User State:", error)
    finally:
        if conn:
            cursor.close()
            conn.close()
...
def get_user_state(chat_id):
    ...
    try:
        cursor = conn.cursor()
        select_query = f"SELECT user_state FROM users WHERE chat_id = '{chat_id}'"
        cursor.execute(select_query)
        user_state = cursor.fetchone()
        return user_state[0] if user_state else None
    except (Exception, Error) as error:
        print("Error while retrieving User State:", error)
