...
OTP_CODE_JOB_NAME = 'otp_code_check_job'

async def check_otp_code_availability(context: CallbackContext) -> None:
    job = context.job
    ...
    if response.status_code == 200:
        if "STATUS_OK" in response.text:
            context.user_data['code_sent'] = True

            respond = response.text.split(':')[1]
            await context.bot.send_message(chat_id=chat_id, text=f"DODE: {respond}")
            
            # await CallbackQuery.edit_message_reply_markup(reply_markup=reply_markup)
            job = context.job_queue.get_jobs_by_name(OTP_CODE_JOB_NAME)[0]
            ...
            await context.bot.send_message(chat_id=chat_id, text="Activation cancelled.")
            job = context.job_queue.get_jobs_by_name(OTP_CODE_JOB_NAME)[0]
            job.schedule_removal()
...
# async def check_code_availability(context: CallbackContext, chat_id: int, api_key: str, id: str, cancel_flag: asyncio.Event) -> None:
#     while not cancel_flag.is_set():
#         response = requests.get(f"https://smstore.su/stubs/handler_api.php?api_key={api_key}&action=getStatus&id={id}")
...
#             respond = response.text.split(':')[1]       
#             await context.bot.send_message(chat_id=chat_id, text=f"FODE: {respond}")
...
async def service_callback(update: Update, context: CallbackContext) -> None:   
    ...
    if responded == "NO_BALANCE":
        ...
        sent_message =  context.bot.send_message(chat_id=query.message.chat_id, text=f"{message}\n\nNumber: \U0001F4F1 {access_number}\nID:{main_id}", reply_markup=reply_markup)
        await sent_message
        job = context.job_queue.get_jobs_by_name(OTP_CODE_JOB_NAME)
        if not job:
            job = context.job_queue.run_repeating(
                check_otp_code_availability,
                interval=2,
                first=0,
                chat_id=chat_id,
                name=OTP_CODE_JOB_NAME
            )
            context.chat_data[OTP_CODE_JOB_NAME] = job
            # await print("I can")
        # await print("I CAN'T")
        # if context.user_data.get('code_sent', False):
        #     sms_keyboard = [
        #         [
        #             InlineKeyboardButton("Request New SMS", callback_data='3'),
        #             InlineKeyboardButton("Check OTP Code", callback_data="otp_code")
        ...
        # job = context.job_queue.run_repeating(check_otp_code_availability, interval=2, first=0, chat_id=chat_id, name=OTP_CODE_JOB_NAME)
        # context.user_data[OTP_CODE_JOB_NAME] = job.name
        ...
        # task = asyncio.create_task(check_otp_code_availability(context, query.message.chat_id, api_key, main_id, cancel_flag))
        
        # await task
        # # Remove "Cancel Activation" button if OTP code was sent
        # if task.result():
...
async def cancel_activation(update: Update, context: CallbackContext):
    ...
    if query.data == "otp_code":
        ...
        if response.status_code == 200 and "ACCESS_CANCEL" in response.text:
        ...
        elif response.status_code == 200 and "ACCESS_WAITING" in response.text:
            response = requests.get(f"https://smstore.su/stubs/handler_api.php?api_key={api_key}&action=getStatus&id={id}")
            respond = response.text.split(':')[1]
            cancel_flag = asyncio.Event()
            # cancel_flag.clear()
        
            await context.bot.send_message(chat_id=query.message.chat_id, text=f"Sending SMS{respond}", reply_markup=reply_markup)            
            # cancel_flag.clear()
            ...
            save_cancel_flag(chat_id, cancel_flag)
            print('here', cancel_flag.is_set())

            # await asyncio.create_task(check_code_availability(context, query.message.chat_id, api_key, id, cancel_flag))
    ...
    #         await context.bot.send_message(chat_id=query.message.chat_id, text="\U0001F534 Activation Cancelled\n\nPress \U0001F3E1 Home to go to main menu\n         ⬅️ Back to go back to services", reply_markup=reply_markup)   
    # elif query.data == '3':
    #     # sms_keyboard = [[InlineKeyboardButton("Request New SMS", callback_data='3'), InlineKeyboardButton("Check OTP Code", callback_data="otp_code")]
    #     #     ] 
